...
void XmlLogger::writeToLogSummary(const SearchResult &sresult)
{
    ...
    element->SetAttribute(CNS_TAG_ATTR_TIMING, sresult.timingInformation.c_str());
    element->SetAttribute(CNS_TAG_ATTR_TOTALEXPANSIONS, std::to_string(sresult.expansions).c_str());
    element->SetAttribute(CNS_TAG_ATTR_STEPS, std::to_string(sresult.steps).c_str());
...
void XmlLogger::writeToLogPath(const SearchResult &sresult, const Task &task, const Config &config)
{
    ...
    for(unsigned int i = 0; i < task.getNumberOfAgents(); i++)
    {
        ...
        {
            XMLElement *part = doc->NewElement("node");
            part->SetAttribute("x",s.j);
