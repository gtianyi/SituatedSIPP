...
auto AA_SIPP::stopCriterion(const Node &curNode, Node &goalNode) -> bool
{
    ...
    if(goalNode.F - CN_EPSILON < curNode.F || (goalNode.g < CN_INFINITY && fabs(goalNode.g - goalNode.interval.begin) < CN_EPSILON)) {
        return true;
}
    return false;
}
...
SearchResult AA_SIPP::startSearch(Map &map, Task &task, DynamicObstacles &obstacles)
{
    ...
    do
    {
        ...
        for(int k = 0; k < task.getNumberOfAgents(); k++)
        {
            ...
            constraints->setParams(curagent.size, curagent.mspeed, curagent.rspeed, config->planforturns, config->inflatecollisionintervals, config->planforturns);
...
bool AA_SIPP::findPath(unsigned int numOfCurAgent, const Map &map)
{
    ...
    Node curNode(curagent.start_i, curagent.start_j, -1, 0, 0), goalNode(curagent.goal_i, curagent.goal_j, -1, CN_INFINITY, CN_INFINITY);
    ...
    while(!stopCriterion(curNode, goalNode))
    {
        ...
        for(auto it = range.first; it != range.second; it++) {
            if(it->second.interval_id == curNode.interval_id)// && it->second.heading_id == curNode.heading_id)
            {
                reexpanded++;
                reexpanded_list.push_back(curNode);
                if(config->planforturns && it->second.heading_id != curNode.heading_id) {
                    reexpanded--;
}
                break;
            }
        ...
        for(Node s:findSuccessors(curNode, map))
        {
            if(config->use_likhachev)
            {
                ...
                bool add = true;
                for(auto it = range.first; it != range.second; it++) {
                    if(it->second.interval_id == curNode.interval_id && (!config->planforturns || it->second.heading_id == curNode.heading_id))
                    {
                        add = false;
                        break;
                    }
