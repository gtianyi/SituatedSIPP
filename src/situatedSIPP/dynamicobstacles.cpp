...
RTDynamicObstacles::RTDynamicObstacles()
{
    obstacles.clear();
}

bool RTDynamicObstacles::getObstacles(const char *fileName)
{
    XMLDocument doc;
    ...
    for(XMLElement *element = root->FirstChildElement(CNS_TAG_OBSTACLE); element; element = element->NextSiblingElement(CNS_TAG_OBSTACLE))
    {
        ...
        for(XMLElement *sec = element->FirstChildElement(CNS_TAG_SECTION); sec; sec = sec->NextSiblingElement(CNS_TAG_SECTION))
        {
            ...
            node.set_static_g(node.g() + sec->DoubleAttribute(CNS_TAG_ATTR_DURATION));
            obs.sections.push_back(node);
        }
...
std::vector<RTNode> RTDynamicObstacles::getSections(int num) const
{
    if(num >= 0 && num < obstacles.size())
        return obstacles[num].sections;
}

double RTDynamicObstacles::getSize(int num) const
{
    if(num >= 0 && num < obstacles.size())
        return obstacles[num].size;
}

double RTDynamicObstacles::getMSpeed(int num) const
{
    if(num >= 0 && num < obstacles.size())
        return obstacles[num].mspeed;
}

std::string RTDynamicObstacles::getID(int num) const
{
    if(num >= 0 && num < obstacles.size())
