...
RTConstraints::RTConstraints(int width, int height)
{
...
void RTConstraints::resetSafeIntervals(int width, int height)
{
    safe_intervals.resize(height);
    for(int i = 0; i < height; i++)
    {
...
void RTConstraints::updateCellSafeIntervals(std::pair<int, int> cell)
{
...
std::vector<SafeInterval> RTConstraints::getSafeIntervals(RTNode curNode, const std::unordered_multimap<int, RTNode> &close, int w)
{
    ...
    for(unsigned int i = 0; i < safe_intervals[curNode.i][curNode.j].size(); i++)
                ...
                && safe_intervals[curNode.i][curNode.j][i].begin <= (curNode.Parent->interval.end + curNode.g() - curNode.Parent->g()))
        {
            ...
            for(auto it = range.first; it != range.second; it++)
                if(it->second.interval.id == safe_intervals[curNode.i][curNode.j][i].id)
                if((use_likhachev && (!planforturns || it->second.heading_id == curNode.heading_id) && it->second.optimal == curNode.optimal) || it->second.g() - it->second.interval.begin < CN_EPSILON
                        || (it->second.g() + tweight*std::min(360 - fabs(curNode.heading - it->second.heading)/(180*rspeed), fabs(curNode.heading - it->second.heading)/(180*rspeed)) - curNode.g()) < CN_EPSILON)//take into account turning cost
                {
                    has = true;
                    break;
                }
...
std::vector<SafeInterval> RTConstraints::getSafeIntervals(RTNode curNode)
{
    return safe_intervals[curNode.i][curNode.j];
}

void RTConstraints::addStartConstraint(int i, int j, int size, std::vector<std::pair<int, int> > cells, double agentsize)
{
...
void RTConstraints::removeStartConstraint(std::vector<std::pair<int, int> > cells, int start_i, int start_j)
{
    for(auto cell: cells)
...
void RTConstraints::addConstraints(const std::vector<RTNode> &sections, double size, double mspeed, const Map &map)
{
...
std::vector<SafeInterval> RTConstraints::findIntervals(RTNode curNode, std::vector<double> &EAT, const std::unordered_multimap<int, RTNode> &close, const Map &map)
{
...
bool RTConstraints::hasCollision(const RTNode &curNode, double startTimeA, const section &constraint, bool &goal_collision)
{
