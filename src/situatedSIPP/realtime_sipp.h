...
public:
    ...
    SearchResult startSearch(Map& map, Task& task, DynamicObstacles& obstacles, SafeIntervals & safe_intervals);
    RTSearchResult startRTSearch(Map& map, Task& task,
                                 DynamicObstacles& obstacles, SafeIntervals& safe_intervals);
    bool   findPath(unsigned int numOfCurAgent, const Map& map, SafeIntervals& safe_intervals) override;
    RTNode findMin();
    ...
    std::list<RTNode> findSuccessors(const RTNode & curNode, const Map& map, SafeIntervals& safe_intervals, const RTNode & goalNode);
